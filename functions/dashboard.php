...
function checarAdm($valorid)
{
    $conn = conectar();
    try {
        $conn->beginTransaction();
        $sqlLista = $conn->prepare("SELECT idadm FROM tbadm WHERE idusuarios = ?");
        $sqlLista->bindValue(1, $valorid, PDO::PARAM_INT);
        $sqlLista->execute();
        $conn->commit();
        if ($sqlLista->rowCount() > 0) {
            return $sqlLista->fetchAll(PDO::FETCH_OBJ);
...
function listarRegistroUInt($tabela, $campos, $idcampo, $idparametro)
{
    $conn = conectar();
    try {
        $conn->beginTransaction();
...
function verificarLogin($email, $pass)
{
    ...
    try {
        $conn->beginTransaction();
        $loginInfo = $conn->prepare("SELECT idusuarios, usuario, senha, ativo FROM tbusuarios WHERE email = ?");
        $loginInfo->bindValue(1, $email, PDO::PARAM_STR);
        $loginInfo->execute();
        $conn->commit();
        if ($loginInfo->rowCount() > 0) {
            $loginDados = $loginInfo->fetch(PDO::FETCH_ASSOC);
            if ($loginDados['ativo'] == 'A') {
                $idUser = $loginDados['idusuarios'];
                $nomeUser = $loginDados['usuario'];
                $senhaUser = $loginDados['senha'];
            ...
            return 'Usuário não existente no banco de dados.';
        }
    } catch (PDOException $e) {
        echo 'ERRO: ';
        return ($e->getMessage());
